...
update(S) <- S - n_SE - n_SVx
update(E) <- E + n_SE - n_Eout
update(I) <- I + n_EoutI - n_Iout

#update(A) <- A + n_EoutA - n_AR - n_AVx
update(A) <- A + n_EoutA - n_AR

update(H) <- H + n_IoutH - n_Hout
update(Q) <- Q + n_HoutQ - n_Qout
update(D) <- D + n_QoutD
update(R) <- R + n_IoutR + n_HoutR + n_QoutR + n_AR - n_RVx

#update(Vx) <- Vx + n_SVx + n_RVx + n_AVx
update(Vx) <- Vx + n_SVx + n_RVx

#update(Vx) <- min(Vx + n_SVx + n_RVx,9e6)
...
update(R_out) <- R_out + n_IoutR + n_HoutR + n_QoutR + n_AR
#update(R_vx) <- R_vx + n_RVx + n_AVx
update(R_vx) <- R_vx + n_RVx
...
#output(Vx_new) <- n_SVx + n_RVx + n_AVx
output(Vx_new) <- n_SVx + n_RVx

#output(sero_vx) <- (n_RVx + n_AVx) / (n_SVx + n_RVx + n_AVx)
output(sero_vx) <- (n_RVx) / (n_SVx + n_RVx)
...
p_SVx <- 1 - exp(-nvx/(S + x*R))
p_RVx <- 1 - exp(-nvx/(S + x*R))

# p_SVx <- 1 - exp(-nvx/(S + x*R + x*A))
# p_RVx <- 1 - exp(-nvx/(S + x*R + x*A))
#p_AVx <- 1 - exp(-nvx/(S + x*R + x*A))
...
### R to Vx
n_RVx <- rbinom(x*R, p_RVx)

### S to Vx
n_SVx <- rbinom(S, p_SVx)

### A to Vx
#n_AVx <- rbinom(x*A, p_AVx)
...
####################################################
### TIME VARYING NUMBER VACCINATED nvx ###
####################################################

nvx <- interpolate(nvx_t, nvx_y,"linear")
nvx_t[] <- user()
nvx_y[] <- user()
dim(nvx_t) <- user()
dim(nvx_y) <- user()
...
##############################################################
### Fraction of sero+ (from R or A) that get vaccinated, x ###
##############################################################
